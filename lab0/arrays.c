...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer: fillArray will fill the blocks of memory past the amount allocated
  // in declaration. With 11 the program runs fine but with 100 it goes too far
  ...
  // Answer: fillArray expects an address of an int so &value will be treated
  ...
  // C++ programming.
  // Answer: Valgrind reports that 20 bytes are leaked on program exit. This
  // means that 20 bytes of memory are still being used despite the program no
  // longer running.
  free(heap_array);
  ...
  FourInts* four_struct = (FourInts*) malloc(sizeof(FourInts));
  fillArray((int*) four_struct, 4);
  assert(four_struct->a == 2);
  assert(four_struct->b == 5);
  assert(four_struct->c == 8);
  assert(four_struct->d == 11);
  free(four_struct);
  return 0;
}
