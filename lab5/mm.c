...
//  printf("Allocating size: %d\n", size);
  ...
  if( (blockSize - reqSize) < MIN_BLOCK_SIZE) {
    followingBlock = (BlockInfo *)POINTER_ADD(ptrFreeBlock, blockSize);
    followingBlock->sizeAndTags |= TAG_PRECEDING_USED;
  }
  else {
    precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;
    ptrFreeBlock->sizeAndTags = reqSize | precedingBlockUseTag | TAG_USED;
    followingBlock = (BlockInfo *)POINTER_ADD(ptrFreeBlock, reqSize);
    followingBlock->sizeAndTags = (blockSize-reqSize) | TAG_PRECEDING_USED;
    //set boundary tag
    ((BlockInfo *)POINTER_ADD(followingBlock, (blockSize-reqSize)-WORD_SIZE))->sizeAndTags = (blockSize-reqSize) | TAG_PRECEDING_USED;
    insertFreeBlock(followingBlock);
  }
...
void mm_free (void *ptr) {
  ...
  payloadSize = (size_t)SIZE(blockInfo->sizeAndTags);
  followingBlock = (BlockInfo *)POINTER_ADD(blockInfo, payloadSize);
  
  //set the header tag
  prevLastWordMask = blockInfo->sizeAndTags & TAG_PRECEDING_USED;
  blockInfo->sizeAndTags = payloadSize | prevLastWordMask;
  //set the boundary tag
  ((BlockInfo*)POINTER_ADD(blockInfo, payloadSize - WORD_SIZE))->sizeAndTags = payloadSize | prevLastWordMask;
  //set the header tag of following block
  followingBlock->sizeAndTags ^= TAG_PRECEDING_USED;
...
int mm_check() {
  int *ptr = mm_malloc(sizeof(int)*4);
  examine_heap();
  int *ptr2 = mm_malloc(sizeof(int)*100);
  examine_heap();
  mm_free(ptr);
  examine_heap();
  mm_free(ptr2);
